...
(function () {
  ...
  if (typeof window.showView !== 'function') {
    window.showView = function (viewName, resetCategory) {
      ...
      if (normalized === 'wrongbook') {
        // WrongBook initialization is handled by onViewActivated in navigateToView
        // Just ensure the lazy group is loaded
        if (window.AppLazyLoader && typeof window.AppLazyLoader.ensureGroup === 'function') {
          window.AppLazyLoader.ensureGroup('practice-suite').catch(function () {});
        }
      }
      if (normalized === 'vocab') {
        // Vocab initialization is handled by onViewActivated in navigateToView
        // Just ensure the lazy group is loaded
        if (window.AppLazyLoader && typeof window.AppLazyLoader.ensureGroup === 'function') {
          window.AppLazyLoader.ensureGroup('more-tools').catch(function () {});
        }
        // Do NOT call VocabMainView.mount here - let onViewActivated handle it
      }
    };
  }
    ...
    if (typeof window.ensureLegacyNavigationController === 'function') {
      window.ensureLegacyNavigationController({
        ...
        onNavigate: function onNavigate(viewName) {
          // Prefer navigateToView (app object) over showView to avoid double-toggle
          if (window.app && typeof window.app.navigateToView === 'function') {
            // navigateToView will handle everything including onViewActivated
            // Only call if currentView is not already this view (avoid duplicate)
            if (window.app.currentView !== viewName) {
        ...
        onViewActivated(viewName) {
            switch (viewName) {
                ...
                case 'practice':
                    ...
                    break;
                case 'wrongbook':
                    // Eagerly load practice-suite group
                    if (window.AppLazyLoader && typeof window.AppLazyLoader.ensureGroup === 'function') {
                        window.AppLazyLoader.ensureGroup('practice-suite').catch(function () {});
                    }
                    if (window.WrongBookView && typeof window.WrongBookView.scheduleRefresh === 'function') {
                        window.WrongBookView.scheduleRefresh(0);
                    } else {
                        var _wbRetry = 0;
                        var _wbTimer = setInterval(function () {
                            _wbRetry++;
                            if (window.WrongBookView && typeof window.WrongBookView.scheduleRefresh === 'function') {
                                clearInterval(_wbTimer);
                                window.WrongBookView.scheduleRefresh(0);
                            } else if (_wbRetry > 50) {
                                clearInterval(_wbTimer);
                            }
                    ...
                    break;
                case 'vocab':
                    // Eagerly load more-tools group
                    if (window.AppLazyLoader && typeof window.AppLazyLoader.ensureGroup === 'function') {
                        window.AppLazyLoader.ensureGroup('more-tools').catch(function () {});
                    }
                    if (window.VocabMainView && typeof window.VocabMainView.mount === 'function') {
                        window.VocabMainView.mount('#vocab-view');
                        window.VocabMainView.refresh();
                    } else {
                        var _vocabRetry = 0;
                        var _vocabTimer = setInterval(function () {
                            _vocabRetry++;
                            if (window.VocabMainView && typeof window.VocabMainView.mount === 'function') {
                                clearInterval(_vocabTimer);
                                window.VocabMainView.mount('#vocab-view');
                                window.VocabMainView.refresh();
                            } else if (_vocabRetry > 50) {
                                clearInterval(_vocabTimer);
                            }
                        }, 200);
                    }
  ...
  const DEFAULT_STEPS = [
    {
      id: 'welcome',
      target: null,
      title: '👋 欢迎使用 HONGHU YASI',
      content: '这是 HONGHU YASI 雅思备考练习系统，提供阅读、听力练习和词汇背诵等功能。让我们快速了解一下各项功能吧！',
      position: 'center',
      showSkip: false,
      showPrev: false,
      nextText: '开始探索',
    ...
    },
    {
      ...
      target: null,
      title: '🎉 恭喜完成！',
      content: '您已了解系统的所有核心功能。现在开始您的 HONGHU YASI 雅思备考之旅吧！祝您取得理想的成绩。',
      position: 'center',
      showSkip: false,
      showPrev: true,
      nextText: '开始练习',
...
/* ===== js/app/wrongBookView.js ===== */
(function initWrongBookView(global) {
    'use strict';
    ...
    function getTypeLabel(type) {
        return QUESTION_TYPE_LABELS[type] || type || '未知';
    }
    ...
    function openReadingExamWithWrongQuestions(examId, mode, wrongQuestionIds) {
        var launchOptions = {
            practiceMode: mode,
        ...
        if (global.app && typeof global.app.openExam === 'function') {
            try {
                ...
                console.warn('[WrongBook] app.openExam 调用失败，尝试降级:', err);
            }
        }
        if (typeof global.openExam === 'function') {
            try {
                ...
                console.warn('[WrongBook] openExam 调用失败，尝试降级:', err);
            }
        }
            ...
            if (manifest && typeof manifest === 'object') {
                for (var key in manifest) {
                    if (manifest.hasOwnProperty(key)) {
                        var m = manifest[key];
                        if (m && (m.examId === examId || m.dataKey === examId)) {
                            exam = m;
                            break;
                        }
                    }
        ...
        var baseUrl = (typeof global.buildBasePath === 'function')
            ? global.buildBasePath()
            : '';
        var url = baseUrl + 'assets/generated/reading-exams/reading-practice-unified.html';
        var params = [];
        params.push('examId=' + encodeURIComponent(examId));
        params.push('dataKey=' + encodeURIComponent(examId));
        params.push('practiceMode=' + encodeURIComponent(mode));
        if (mode === 'wrongbook' || mode === 'wrongonly') {
            params.push('wrongQuestionIds=' + encodeURIComponent(wrongQuestionIds.join(',')));
        }
        url += '?' + params.join('&');
        var absoluteUrl = (typeof global.buildAbsoluteUrl === 'function')
            ? global.buildAbsoluteUrl(url)
            : url;
        try {
            var features = 'width=1024,height=768,scrollbars=yes,resizable=yes';
            var win = window.open(absoluteUrl, 'exam_' + examId, features);
            if (win) {
                win.focus();
                return;
            }
        } catch (_) {}
        global.location.href = absoluteUrl;
    }

    function renderWrongBookStats(stats) {
        var elTotal = document.getElementById('wrongbook-total');
        var elUnmastered = document.getElementById('wrongbook-unmastered');
        var elMastered = document.getElementById('wrongbook-mastered');
        var elExamCount = document.getElementById('wrongbook-exam-count');
        if (elTotal) elTotal.textContent = stats.total;
        if (elUnmastered) elUnmastered.textContent = stats.unmastered;
        if (elMastered) elMastered.textContent = stats.mastered;

        var elHalfway = document.getElementById('wrongbook-halfway');
        if (elHalfway) elHalfway.textContent = stats.halfway || 0;

        var examSet = new Set();
        wrongBookState.groups.forEach(function (g) { examSet.add(g.examId); });
        if (elExamCount) elExamCount.textContent = examSet.size;
    }

    function renderWrongBookList() {
        var container = document.getElementById('wrongbook-list');
        if (!container) return;

        var groups = wrongBookState.groups.slice();

        if (wrongBookState.filterCategory !== 'all') {
            groups = groups.filter(function (g) {
                return g.category === wrongBookState.filterCategory;
            });
        }

        if (wrongBookState.filterMastery !== 'all') {
            groups = groups.filter(function (g) {
                if (wrongBookState.filterMastery === 'mastered') return g.unmastered === 0 && g.mastered > 0;
                if (wrongBookState.filterMastery === 'halfway') return g.halfway > 0;
                if (wrongBookState.filterMastery === 'unmastered') return g.unmastered > 0;
                return true;
            });
        }

        if (wrongBookState.sortMode === 'count') {
            groups.sort(function (a, b) { return b.totalWrong - a.totalWrong; });
        } else if (wrongBookState.sortMode === 'category') {
            groups.sort(function (a, b) {
                var catOrder = { 'P1': 1, 'P2': 2, 'P3': 3 };
        ...
        if (groups.length === 0) {
            container.innerHTML =
                ...
                '<div class="wrongbook-empty__icon">📋</div>' +
                '<p>暂无错题记录</p>' +
                '<p class="wrongbook-empty__note">练习中答错的题目将自动收录在此</p>' +
                '</div>';
            return;
        }
        ...
        groups.forEach(function (group) {
            var allMastered = group.unmastered === 0;
            var statusBadge = allMastered
                ? '<span class="wrongbook-card__badge wrongbook-card__badge--mastered">✅ 已全部掌握</span>'
                : '<span class="wrongbook-card__badge wrongbook-card__badge--unmastered">❌ ' + group.unmastered + '题未掌握' + (group.halfway > 0 ? ' · 🔶 ' + group.halfway + '题对1次' : '') + '</span>';

            var categoryBadge = group.category
                ? '<span class="wrongbook-card__badge wrongbook-card__badge--' + group.category + '">' + group.category + '</span>'
                : '';
            ...
            group.questions.forEach(function (q) {
                var cls, icon;
                if (q.mastered) {
                    cls = 'wrongbook-question-chip--mastered';
                    icon = '✅';
                } else if (q.correctCount >= 1) {
                    cls = 'wrongbook-question-chip--halfway';
                    icon = '🔶';
                } else {
                    cls = 'wrongbook-question-chip--unmastered';
                    icon = '❌';
                }
                questionChips +=
                    '<span class="wrongbook-question-chip ' + cls + '" data-composite-key="' + q.compositeKey + '" title="' + getTypeLabel(q.questionType) + ': 你的答案=' + q.userAnswer + ' 正确答案=' + q.correctAnswer + ' 重练对' + (q.correctCount || 0) + '次">' +
                    icon + ' ' + q.questionId +
                    '<span class="wrongbook-question-chip__type">' + getTypeLabel(q.questionType) + '</span>' +
                    '</span>';
            });

            var wrongIds = group.questions
                .filter(function (q) { return !q.mastered; })
                .map(function (q) { return q.questionId; });

            var allWrongIds = group.questions.map(function (q) { return q.questionId; });

            var fullBtn = '<button class="btn wrongbook-btn--full" data-wrongbook-action="full-retry" data-exam-id="' + group.examId + '" data-wrong-ids="' + allWrongIds.join(',') + '">整卷重练</button>';
            var wrongOnlyBtn = wrongIds.length > 0
                ? '<button class="btn wrongbook-btn--wrongonly" data-wrongbook-action="wrongonly-retry" data-exam-id="' + group.examId + '" data-wrong-ids="' + wrongIds.join(',') + '">仅错题重练</button>'
                : '';
            var aiBtn = '<button class="btn wrongbook-btn--ai" data-wrongbook-action="ai-analyze" data-exam-id="' + group.examId + '">AI分析</button>';

            html +=
                '<div class="wrongbook-card" data-exam-id="' + group.examId + '">' +
                '<div class="wrongbook-card__header">' +
                '<h3 class="wrongbook-card__title">📄 ' + (group.title || group.examId) + '</h3>' +
                '<div>' + categoryBadge + statusBadge + '</div>' +
                '</div>' +
                '<div class="wrongbook-card__meta">错' + group.totalWrong + '题 · 最近出错: ' + formatRelativeTime(group.lastWrongAt) + '</div>' +
                '<div class="wrongbook-card__questions">' + questionChips + '</div>' +
                '<div class="wrongbook-card__actions">' + fullBtn + wrongOnlyBtn + aiBtn + '</div>' +
                '</div>';
        });
        ...
        groups.forEach(function (g) {
            var cached = _analysisCache[g.examId];
            if (cached && cached.parsed) {
                updateCardLabels(g.examId);
                var btn = document.querySelector('.wrongbook-btn--ai[data-exam-id="' + g.examId + '"]');
                if (btn) btn.textContent = '查看报告';
            }
            var ai = global.WrongbookAI;
            if (ai && ai.isPending && ai.isPending(g.examId)) {
                var btn = document.querySelector('.wrongbook-btn--ai[data-exam-id="' + g.examId + '"]');
                if (btn) {
                    btn.classList.add('wrongbook-btn--ai--loading');
                    btn.textContent = '分析中...';
                    btn.disabled = true;
                }
            }
        ...
        var allCached = _analysisCache['__all_exams__'];
        var allBtn = document.getElementById('wrongbook-all-analysis-btn');
        if (allBtn) {
            if (allCached && allCached.parsed) {
                allBtn.textContent = '查看总报告';
            } else {
                var ai = global.WrongbookAI;
                if (ai && ai.isPending && ai.isPending('__all_exams__')) {
                    allBtn.classList.add('wrongbook-btn--ai--loading');
                    allBtn.textContent = '分析中...';
                    allBtn.disabled = true;
                }
            }
        }

        preloadAnalysisLabels(groups.filter(function (g) { return !_analysisCache[g.examId]; }));
    }

    var wrongBookRefreshRetryCount = 0;
    var WRONGBOOK_MAX_RETRIES = 20;
    var _refreshDebounceTimer = null;
    var _pendingRefresh = false;
    var _dirtySinceLastRefresh = false;

    function showWrongBookLoading() {
        var listEl = document.getElementById('wrongbook-list');
        if (!listEl) return;
        listEl.innerHTML =
            '<div class="wrongbook-loading">' +
            '<div class="wrongbook-loading__spinner"></div>' +
            '<p class="wrongbook-loading__text">正在加载错题数据...</p>' +
            '</div>';
        var statsEl = document.getElementById('wrongbook-stats');
        if (statsEl) statsEl.classList.add('wrongbook-stats--loading');
    }

    function hideWrongBookLoading() {
        var statsEl = document.getElementById('wrongbook-stats');
        if (statsEl) statsEl.classList.remove('wrongbook-stats--loading');
    }

    function refreshWrongBook() {
        if (wrongBookState.loading) {
            _pendingRefresh = true;
            return;
        }
        wrongBookState.loading = true;
        _dirtySinceLastRefresh = false;
        showWrongBookLoading();

        var store = global.getWrongAnswerStore && global.getWrongAnswerStore();
        if (!store) {
            if (global.AppLazyLoader && typeof global.AppLazyLoader.ensureGroup === 'function') {
                global.AppLazyLoader.ensureGroup('practice-suite').then(function () {
                    wrongBookState.loading = false;
                    refreshWrongBook();
                }).catch(function () {
                    wrongBookState.loading = false;
                    wrongBookRefreshRetryCount++;
                    if (wrongBookRefreshRetryCount <= WRONGBOOK_MAX_RETRIES) {
                        var delay = Math.min(500 * wrongBookRefreshRetryCount, 3000);
                        console.warn('[WrongBook] 加载 practice-suite 失败，' + delay + 'ms 后重试');
                        setTimeout(refreshWrongBook, delay);
                    } else {
                        console.warn('[WrongBook] practice-suite 不可用，已达最大重试次数');
                        renderWrongBookStats({ total: 0, mastered: 0, unmastered: 0 });
                        var listEl = document.getElementById('wrongbook-list');
                        if (listEl) {
                            listEl.innerHTML =
                                '<div class="wrongbook-loading wrongbook-loading--error">' +
                                '<div class="wrongbook-loading__icon">⚠️</div>' +
                                '<p class="wrongbook-loading__text">错题数据加载失败</p>' +
                                '<p class="wrongbook-loading__note">请刷新页面重试</p>' +
                                '</div>';
                        }
                        wrongBookRefreshRetryCount = 0;
                        hideWrongBookLoading();
                    }
                });
            } else {
                wrongBookState.loading = false;
                ...
                if (wrongBookRefreshRetryCount <= WRONGBOOK_MAX_RETRIES) {
                    var delay = Math.min(500 * wrongBookRefreshRetryCount, 3000);
                    console.warn('[WrongBook] WrongAnswerStore 尚未加载，' + delay + 'ms 后重试 (' + wrongBookRefreshRetryCount + '/' + WRONGBOOK_MAX_RETRIES + ')');
                    setTimeout(refreshWrongBook, delay);
                } else {
                    console.warn('[WrongBook] WrongAnswerStore 不可用，已达最大重试次数');
                    renderWrongBookStats({ total: 0, mastered: 0, unmastered: 0 });
                    var listEl2 = document.getElementById('wrongbook-list');
                    if (listEl2) {
                        listEl2.innerHTML =
                            '<div class="wrongbook-loading wrongbook-loading--error">' +
                            '<div class="wrongbook-loading__icon">⚠️</div>' +
        ...
        var dbReady = (typeof store.ensureDB === 'function')
            ? store.ensureDB()
        ...
        dbReady.then(function () {
            ...
            var p = (typeof store.getGroupedWithStats === 'function')
                ? store.getGroupedWithStats()
                : Promise.all([store.getGroupedByExam(), store.getStats()]).then(function (r) {
                    return { groups: r[0] || [], stats: r[1] || { total: 0, mastered: 0, unmastered: 0 } };
                });

            return p.then(function (result) {
                wrongBookState.groups = result.groups || [];
                renderWrongBookStats(result.stats || { total: 0, mastered: 0, unmastered: 0 });
                renderWrongBookList();
            });
        }).catch(function (err) {
            console.error('[WrongBook] 加载错题数据失败:', err);
            wrongBookRefreshRetryCount++;
            if (wrongBookRefreshRetryCount <= WRONGBOOK_MAX_RETRIES) {
                var retryDelay = Math.min(500 * wrongBookRefreshRetryCount, 3000);
                console.warn('[WrongBook] ' + retryDelay + 'ms 后重试 (' + wrongBookRefreshRetryCount + '/' + WRONGBOOK_MAX_RETRIES + ')');
                setTimeout(refreshWrongBook, retryDelay);
            } else {
                var listEl2 = document.getElementById('wrongbook-list');
                if (listEl2) {
                    listEl2.innerHTML =
                        '<div class="wrongbook-loading wrongbook-loading--error">' +
                        '<div class="wrongbook-loading__icon">⚠️</div>' +
                        '<p class="wrongbook-loading__text">加载失败，请稍后重试</p>' +
                        '</div>';
                }
                wrongBookRefreshRetryCount = 0;
            }
    ...
    function scheduleRefresh(delay) {
        var wrongbookView = document.getElementById('wrongbook-view');
        var isActive = wrongbookView && (wrongbookView.classList.contains('active') || wrongbookView.offsetParent !== null);
        if (document.hidden || !isActive) {
            _dirtySinceLastRefresh = true;
            if (_refreshDebounceTimer) { clearTimeout(_refreshDebounceTimer); _refreshDebounceTimer = null; }
            return;
        }
    ...
    global.addEventListener('visibilitychange', function () {
        if (!document.hidden && _dirtySinceLastRefresh) {
            scheduleRefresh(0);
        }
    });

    global.addEventListener('focus', function () {
        if (_dirtySinceLastRefresh) {
            scheduleRefresh(100);
        }
    });

    global.addEventListener('wrongbook:data-changed', function () {
        scheduleRefresh(150);
    });

    var _wbViewEl = document.getElementById('wrongbook-view');
    if (_wbViewEl && typeof MutationObserver !== 'undefined') {
        var _wbObserver = new MutationObserver(function (mutations) {
        ...
        _wbObserver.observe(_wbViewEl, { attributes: true, attributeFilter: ['class'] });
    }

    function renderAnalysisResult(data, examId, timeLabel) {
        if (!data || !data.parsed) {
            var rawHtml = String(data && data.raw || '');
            rawHtml = rawHtml.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>');
            return '<div class="wrongbook-analysis__body">' + rawHtml + '</div>' +
                '<div class="wrongbook-analysis__footer">' +
                '<button class="btn btn-sm btn-outline" data-wrongbook-action="ai-retry" data-exam-id="' + examId + '">重新分析</button>' +
                '</div>';
        }

        return renderParsedAnalysis(data.parsed, examId);
    }

    function renderParsedAnalysis(p, examId) {
        var bodyHtml = '';

        if (p.summary) {
            var levelColors = { weak: '#dc2626', 'below-average': '#f59e0b', average: '#3b82f6', 'above-average': '#16a34a' };
            var levelLabels = { weak: '较弱', 'below-average': '偏下', average: '中等', 'above-average': '较好' };
            var levelColor = levelColors[p.summary.overallLevel] || '#6b7280';
            var levelLabel = levelLabels[p.summary.overallLevel] || p.summary.overallLevel || '未知';
            bodyHtml += '<div class="wrongbook-analysis__section wrongbook-analysis__summary-section">';
            bodyHtml += '<div class="wrongbook-analysis__summary-level" style="border-left:4px solid ' + levelColor + '">' +
                '<span class="wrongbook-analysis__summary-level-label" style="color:' + levelColor + '">' + escHtml(levelLabel) + '</span>' +
                '<span class="wrongbook-analysis__summary-level-text">整体阅读能力</span></div>';
            if (p.summary.keyInsight) {
                bodyHtml += '<div class="wrongbook-analysis__summary-insight">💡 ' + escHtml(p.summary.keyInsight) + '</div>';
            }
            if (p.summary.overallLevelDescription) {
                bodyHtml += '<div class="wrongbook-analysis__summary-desc">' + escHtml(p.summary.overallLevelDescription) + '</div>';
            }
            if (p.summary.examCount && p.summary.examCount > 1) {
                bodyHtml += '<div class="wrongbook-analysis__summary-meta">涵盖 ' + p.summary.examCount + ' 套试卷 · 共 ' + (p.summary.totalQuestions || 0) + ' 道错题</div>';
            }
            bodyHtml += '</div>';
        }

        if (p.patternAnalysis) {
            var pa = p.patternAnalysis;
            bodyHtml += '<div class="wrongbook-analysis__section"><h4>📊 错题模式分析</h4>';
            if (pa.errorTypeDistribution) {
                var entries = Object.entries(pa.errorTypeDistribution).filter(function (e) { return e[1] > 0; }).sort(function (a, b) { return b[1] - a[1]; });
                if (entries.length > 0) {
                    var maxVal = entries[0][1];
                    bodyHtml += '<div class="wrongbook-analysis__chart">';
                    entries.forEach(function (e) {
                        var pct = Math.round((e[1] / maxVal) * 100);
                        bodyHtml += '<div class="wrongbook-analysis__chart-row">' +
                            '<span class="wrongbook-analysis__chart-label">' + escHtml(e[0]) + '</span>' +
                            '<div class="wrongbook-analysis__chart-bar-wrap"><div class="wrongbook-analysis__chart-bar" style="width:' + pct + '%"></div></div>' +
                            '<span class="wrongbook-analysis__chart-val">' + e[1] + '</span></div>';
                    });
                    bodyHtml += '</div>';
                }
            }
            if (pa.patternSummary) {
                bodyHtml += '<div class="wrongbook-analysis__chart-summary">' + escHtml(pa.patternSummary) + '</div>';
            }
            if (pa.topErrors && pa.topErrors.length > 0) {
                bodyHtml += '<div class="wrongbook-analysis__top-errors">';
                pa.topErrors.slice(0, 3).forEach(function (te) {
                    bodyHtml += '<span class="wrongbook-analysis__top-error-chip">' + escHtml(te.errorType || '') + ' ' + (te.count || 0) + '次 (' + (te.percentage || 0) + '%)</span>';
                });
                bodyHtml += '</div>';
            }
            bodyHtml += '</div>';
        ...
        if (p.questionAnalyses && p.questionAnalyses.length > 0) {
            bodyHtml += '<div class="wrongbook-analysis__section"><h4>🔍 逐题精析</h4>';
            p.questionAnalyses.forEach(function (qa) {
                var labelsHtml = (qa.labels || []).map(function (l) {
                    return '<span class="wrongbook-label-chip">' + escHtml(l) + '</span>';
                }).join('');
                var diffColors = { easy: '#16a34a', medium: '#f59e0b', hard: '#dc2626' };
                var diffLabels = { easy: '易', medium: '中', hard: '难' };
                var diffHtml = qa.difficulty ? '<span class="wrongbook-difficulty-chip" style="background:' + (diffColors[qa.difficulty] || '#6b7280') + '">' + escHtml(diffLabels[qa.difficulty] || qa.difficulty) + '</span>' : '';
                bodyHtml +=
                    '<div class="wrongbook-analysis__question">' +
                    '<div class="wrongbook-analysis__question-header">' +
                    '<span class="wrongbook-analysis__q-id">' + escHtml(qa.questionId || '') + '</span>' +
                    '<span class="wrongbook-analysis__q-type">' + escHtml(qa.questionType || '') + '</span>' +
                    diffHtml +
                    '</div>' +
                    '<div class="wrongbook-analysis__q-answers">' +
                    '<span class="wrongbook-analysis__q-user">你的答案: <strong>' + escHtml(qa.userAnswer || '') + '</strong></span>' +
                    '<span class="wrongbook-analysis__q-correct">正确答案: <strong>' + escHtml(qa.correctAnswer || '') + '</strong></span>' +
                    '</div>' +
                    '<div class="wrongbook-analysis__q-labels">' + labelsHtml + '</div>' +
                    '<div class="wrongbook-analysis__q-analysis">' + escHtml(qa.analysis || '') + '</div>' +
                    (qa.originalSentence ? '<div class="wrongbook-analysis__q-original">📖 原文关键句: ' + escHtml(qa.originalSentence) + '</div>' : '') +
                    (qa.tip ? '<div class="wrongbook-analysis__q-tip">💡 技巧: ' + escHtml(qa.tip) + '</div>' : '') +
                    '</div>';
            });
            bodyHtml += '</div>';
        }
        ...
        if (p.weakPoints && p.weakPoints.length > 0) {
            bodyHtml += '<div class="wrongbook-analysis__section"><h4>⚠️ 薄弱点诊断</h4>';
            p.weakPoints.forEach(function (wp) {
                var sevClass = 'wrongbook-severity--' + (wp.severity || 'medium');
                var qRefs = (wp.affectedQuestions || []).map(function (q) { return escHtml(q); }).join(', ');
                bodyHtml +=
                    '<div class="wrongbook-analysis__weak-point">' +
                    '<div class="wrongbook-analysis__wp-header">' +
                    '<span class="wrongbook-severity ' + sevClass + '">' + escHtml(wp.severity || 'medium') + '</span>' +
                    '<span class="wrongbook-analysis__wp-point">' + escHtml(wp.point || '') + '</span>' +
                    '</div>' +
                    '<div class="wrongbook-analysis__wp-desc">' + escHtml(wp.description || '') + '</div>' +
                    (wp.rootCause ? '<div class="wrongbook-analysis__wp-root">根因: ' + escHtml(wp.rootCause) + '</div>' : '') +
                    (wp.impact ? '<div class="wrongbook-analysis__wp-impact">影响: ' + escHtml(wp.impact) + '</div>' : '') +
                    (qRefs ? '<div class="wrongbook-analysis__wp-refs">涉及题号: ' + qRefs + '</div>' : '') +
                    '</div>';
            });
            bodyHtml += '</div>';
        }
        ...
        if (p.studyPlan) {
            var sp = p.studyPlan;
            bodyHtml += '<div class="wrongbook-analysis__section"><h4>📅 分阶段提升计划</h4>';
            var phases = [
                { key: 'shortTerm', label: '短期', icon: '🏃' },
                { key: 'midTerm', label: '中期', icon: '📈' },
                { key: 'longTerm', label: '长期', icon: '🎯' }
            ];
            phases.forEach(function (phase) {
                var plan = sp[phase.key];
                if (!plan) return;
                bodyHtml += '<div class="wrongbook-analysis__plan-phase">' +
                    '<div class="wrongbook-analysis__plan-header">' + phase.icon + ' ' + phase.label + ' (' + escHtml(plan.period || '') + ')</div>';
                if (plan.goals && plan.goals.length > 0) {
                    bodyHtml += '<div class="wrongbook-analysis__plan-goals">' + plan.goals.map(function (g) { return '<span class="wrongbook-plan-goal">✓ ' + escHtml(g) + '</span>'; }).join('') + '</div>';
                }
                if (plan.actions && plan.actions.length > 0) {
                    bodyHtml += '<div class="wrongbook-analysis__plan-actions">' + plan.actions.map(function (a) { return '<span class="wrongbook-plan-action">→ ' + escHtml(a) + '</span>'; }).join('') + '</div>';
                }
                bodyHtml += '</div>';
            });
            bodyHtml += '</div>';
        ...
        if (p.suggestions && p.suggestions.length > 0) {
            bodyHtml += '<div class="wrongbook-analysis__section"><h4>💡 改进建议</h4>';
            p.suggestions.forEach(function (s) {
                var priColors = { high: '#dc2626', medium: '#f59e0b', low: '#6b7280' };
                var priHtml = s.priority ? '<span class="wrongbook-priority-chip" style="background:' + (priColors[s.priority] || '#6b7280') + '">' + escHtml(s.priority) + '</span>' : '';
                bodyHtml +=
                    '<div class="wrongbook-analysis__suggestion">' +
                    priHtml +
                    '<div class="wrongbook-analysis__sug-target">针对: ' + escHtml(s.target || '') + '</div>' +
                    '<div class="wrongbook-analysis__sug-method">方法: ' + escHtml(s.method || '') + '</div>' +
                    '<div class="wrongbook-analysis__sug-dir">方向: ' + escHtml(s.direction || '') + '</div>' +
                    (s.notes ? '<div class="wrongbook-analysis__sug-notes">注意: ' + escHtml(s.notes) + '</div>' : '') +
                    '</div>';
            });
            bodyHtml += '</div>';
        }

        return '<div class="wrongbook-analysis__body">' + bodyHtml + '</div>' +
            '<div class="wrongbook-analysis__footer">' +
            '<button class="btn btn-sm btn-outline" data-wrongbook-action="ai-retry" data-exam-id="' + examId + '">重新分析</button>' +
            '</div>';
    }
    ...
    var _reportWindows = {};
    function openAnalysisModal(data, examId, timeLabel) {
        var baseUrl = 'assets/generated/reading-exams/ai-report.html?examId=' + encodeURIComponent(examId);
        var screenWidth = window.screen.availWidth;
        var screenHeight = window.screen.availHeight;
        var w = Math.floor(screenWidth * 0.8);
        var h = Math.floor(screenHeight * 0.8);
        var left = Math.floor((screenWidth - w) / 2);
        var top = Math.floor((screenHeight - h) / 2);
        var features = 'width=' + w + ',height=' + h + ',left=' + left + ',top=' + top + ',scrollbars=yes,resizable=yes,status=yes,toolbar=no,menubar=no,location=no';
        var winName = 'ai_report_' + examId;
        var existing = _reportWindows[winName];
        if (existing && !existing.closed) {
            try { existing.location.href = baseUrl; existing.focus(); return; } catch (_) {}
        }
        var reportWin = window.open(baseUrl, winName, features);
        if (reportWin) { _reportWindows[winName] = reportWin; reportWin.focus(); }
    }
    ...
    function getAllLabelsFromAnalysis(parsed) {
        if (!parsed || !parsed.questionAnalyses) return [];
        var labelMap = {};
        parsed.questionAnalyses.forEach(function (qa) {
            (qa.labels || []).forEach(function (l) {
                if (l) labelMap[l] = (labelMap[l] || 0) + 1;
            });
        });
        return Object.keys(labelMap).sort(function (a, b) { return labelMap[b] - labelMap[a]; });
    }

    function renderLabelChips(labels) {
        if (!labels || labels.length === 0) return '';
        return '<div class="wrongbook-card__labels">' +
            labels.map(function (l) { return '<span class="wrongbook-label-chip">' + escHtml(l) + '</span>'; }).join('') +
            '</div>';
    }

    var _analysisCache = {};

    function preloadAnalysisLabels(groups) {
        var ai = global.WrongbookAI;
        if (!ai || typeof ai.getResult !== 'function') return;
        groups.forEach(function (g) {
            ai.getResult(g.examId).then(function (result) {
                if (result && result.parsed) {
                    _analysisCache[g.examId] = result;
                }
            }).catch(function () {});
        });
    }

    function handleAIAnalysis(examId, btnEl) {
        var ai = global.WrongbookAI;
        if (!ai || typeof ai.analyze !== 'function') {
            if (typeof global.showMessage === 'function') {
                global.showMessage('AI分析模块未加载，请稍后重试', 'error');
            }
            return;
        }

        if (btnEl.classList.contains('wrongbook-btn--ai--loading')) return;

        var existing = _analysisCache[examId];
        if (existing && existing.parsed) {
            openAnalysisModal(existing, examId, formatRelativeTime(existing.analyzedAt));
            return;
        }

        btnEl.classList.add('wrongbook-btn--ai--loading');
        btnEl.textContent = '分析中...';
        btnEl.disabled = true;

        ai.analyze(examId).then(function (result) {
            _analysisCache[examId] = { parsed: result.parsed, raw: result.raw, analyzedAt: new Date().toISOString() };
            var b = document.querySelector('.wrongbook-btn--ai[data-exam-id="' + examId + '"]') || btnEl;
            if (b) {
                b.classList.remove('wrongbook-btn--ai--loading');
                b.textContent = '查看报告';
                b.disabled = false;
            }
            updateCardLabels(examId);
            openAnalysisModal(_analysisCache[examId], examId, '刚刚');
        }).catch(function (err) {
            console.error('[WrongBook] AI分析失败:', err);
            var b = document.querySelector('.wrongbook-btn--ai[data-exam-id="' + examId + '"]') || btnEl;
            if (b) {
                b.classList.remove('wrongbook-btn--ai--loading');
                b.textContent = 'AI分析';
            ...
            if (typeof global.showMessage === 'function') {
                global.showMessage('分析失败: ' + (err.message || '未知错误'), 'error');
            }
        });
    }

    function handleAllExamsAnalysis(btnEl) {
        var ai = global.WrongbookAI;
        if (!ai || typeof ai.analyzeAll !== 'function') {
            if (typeof global.showMessage === 'function') {
                global.showMessage('AI分析模块未加载，请稍后重试', 'error');
            }
            return;
        ...
        var existing = _analysisCache['__all_exams__'];
        if (existing && existing.parsed) {
            openAnalysisModal(existing, '__all_exams__', formatRelativeTime(existing.analyzedAt));
            return;
        }
        ...
        ai.analyzeAll().then(function (result) {
            _analysisCache['__all_exams__'] = { parsed: result.parsed, raw: result.raw, analyzedAt: new Date().toISOString() };
            var b = document.getElementById('wrongbook-all-analysis-btn') || btnEl;
            if (b) {
                b.classList.remove('wrongbook-btn--ai--loading');
                b.textContent = '查看总报告';
                b.disabled = false;
            }
            openAnalysisModal(_analysisCache['__all_exams__'], '__all_exams__', '刚刚');
        }).catch(function (err) {
            console.error('[WrongBook] 总错题分析失败:', err);
            var b = document.getElementById('wrongbook-all-analysis-btn') || btnEl;
            if (b) {
                b.classList.remove('wrongbook-btn--ai--loading');
                b.textContent = '总错题分析';
    ...
    function updateCardLabels(examId) {
        var data = _analysisCache[examId];
        if (!data || !data.parsed) return;
        var labels = getAllLabelsFromAnalysis(data.parsed);
        var card = document.querySelector('.wrongbook-card[data-exam-id="' + examId + '"]');
        if (!card) return;
        var existingLabels = card.querySelector('.wrongbook-card__labels');
        if (existingLabels) existingLabels.remove();
        var meta = card.querySelector('.wrongbook-card__meta');
        if (meta) {
            var labelHtml = renderLabelChips(labels);
            if (labelHtml) meta.insertAdjacentHTML('afterend', labelHtml);
        }
    }

    function handleWrongBookClick(e) {
        var target = e.target.closest('[data-wrongbook-action]');
        if (!target) {
            var chip = e.target.closest('[data-composite-key]');
            if (chip) {
                var compositeKey = chip.getAttribute('data-composite-key');
                var store = global.getWrongAnswerStore && global.getWrongAnswerStore();
                if (store && compositeKey) {
                    store.toggleMastered(compositeKey);
                }
            }
            return;
        }

        var action = target.getAttribute('data-wrongbook-action');
        var examId = target.getAttribute('data-exam-id');
        var wrongIdsStr = target.getAttribute('data-wrong-ids') || '';
        var wrongIds = wrongIdsStr.split(',').filter(Boolean);

        if (action === 'full-retry' && examId) {
            openReadingExamWithWrongQuestions(examId, 'wrongbook', wrongIds);
        } else if (action === 'wrongonly-retry' && examId) {
            openReadingExamWithWrongQuestions(examId, 'wrongonly', wrongIds);
        } else if (action === 'clear-all') {
            if (!global.confirm('确定要清除所有错题数据吗？此操作不可撤销。')) return;
            var store = global.getWrongAnswerStore && global.getWrongAnswerStore();
            if (store && typeof store.clearAll === 'function') {
                store.clearAll().then(function () {
                    if (typeof global.showMessage === 'function') {
                        global.showMessage('错题数据已清除', 'success');
                    }
                }).catch(function (err) {
                    console.error('[WrongBook] 清除数据失败:', err);
                    if (typeof global.showMessage === 'function') {
                        global.showMessage('清除失败，请重试', 'error');
                    }
                });
            }
        } else if (action === 'ai-analyze' && examId) {
            handleAIAnalysis(examId, target);
        } else if (action === 'ai-retry' && examId) {
            delete _analysisCache[examId];
            var ai = global.WrongbookAI;
            if (ai && ai.forceNew) ai.forceNew(examId);
            var aiRetryBtn = document.querySelector('.wrongbook-btn--ai[data-exam-id="' + examId + '"]');
            if (aiRetryBtn) {
                aiRetryBtn.textContent = 'AI分析';
                handleAIAnalysis(examId, aiRetryBtn);
            }
        } else if (action === 'all-analysis') {
            handleAllExamsAnalysis(target);
        } else if (action === 'view-report' && examId) {
            var cached = _analysisCache[examId];
            if (cached && cached.parsed) {
                openAnalysisModal(cached, examId, formatRelativeTime(cached.analyzedAt));
            } else {
                handleAIAnalysis(examId, target);
            }
        }
    ...
    function handleWrongBookFilter(e) {
        var btn = e.target.closest('[data-wrongbook-filter]');
        if (!btn) return;

        var filterType = btn.getAttribute('data-wrongbook-filter');
        var filterValue = btn.getAttribute('data-filter-value');

        var group = btn.closest('.wrongbook-filter-group');
        if (group) {
            group.querySelectorAll('.shui-segmented-btn').forEach(function (b) {
        ...
        if (filterType === 'category') {
            wrongBookState.filterCategory = filterValue;
        } else if (filterType === 'mastery') {
            wrongBookState.filterMastery = filterValue;
        }
        renderWrongBookList();
    }

    function initWrongBookEventDelegates() {
        document.addEventListener('click', handleWrongBookClick);
        document.addEventListener('click', handleWrongBookFilter);

        var sortEl = document.getElementById('wrongbook-sort');
        if (sortEl) {
            sortEl.addEventListener('change', function () {
                wrongBookState.sortMode = sortEl.value;
                renderWrongBookList();
            });
        }

        var wrongbookView = document.getElementById('wrongbook-view');
        if (wrongbookView && (wrongbookView.classList.contains('active') || wrongbookView.offsetParent !== null)) {
            scheduleRefresh(0);
            var store = global.getWrongAnswerStore && global.getWrongAnswerStore();
            if (!store && global.AppLazyLoader && typeof global.AppLazyLoader.ensureGroup === 'function') {
                global.AppLazyLoader.ensureGroup('practice-suite').then(function () {
                    scheduleRefresh(0);
                }).catch(function () {});
            }
    ...
    global.WrongBookView = {
        refresh: refreshWrongBook,
        scheduleRefresh: scheduleRefresh,
        init: initWrongBookEventDelegates
    };

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initWrongBookEventDelegates);
    } else {
        initWrongBookEventDelegates();
    }

    console.log('[WrongBookView] 错题本视图模块已注册');
