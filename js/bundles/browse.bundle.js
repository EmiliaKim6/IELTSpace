...
(function (global) {
    ...
    historyRenderer.createRecordNode = function (record, options) {
        ...
        var titleText = record && record.title ? record.title : '无标题';
        var recordPracticeMode = String(record && record.metadata && record.metadata.practiceMode || '').toLowerCase();
        var titleContent = [createNode('strong', null, titleText)];
        if (recordPracticeMode === 'wrongbook') {
            titleContent.push(createNode('span', { className: 'practice-mode-badge practice-mode-badge--wrongbook' }, '整卷重练'));
        } else if (recordPracticeMode === 'wrongonly') {
            titleContent.push(createNode('span', { className: 'practice-mode-badge practice-mode-badge--wrongonly' }, '仅错题'));
        }
        ...
        var percentageNode = createNode('div', { className: 'record-percentage-container' }, [
            createNode('div', {
                className: 'record-percentage' + ((recordPracticeMode === 'wrongbook' || recordPracticeMode === 'wrongonly') ? ' record-percentage--excluded' : ''),
...
(function (global) {
    ...
    const mixin = {
        ...
        _buildUnifiedReadingUrl(exam, options = {}) {
            ...
            } else if (practiceMode === 'wrongbook' || practiceMode === 'wrongonly') {
                params.set('practiceMode', practiceMode);
                const wrongIds = options && options.wrongQuestionIds;
                if (Array.isArray(wrongIds) && wrongIds.length) {
                    params.set('wrongQuestionIds', wrongIds.join(','));
                }
            }
            ...
            try {
                ...
                try {
                    if (typeof window.getWrongAnswerStore === 'function') {
                        var was2 = window.getWrongAnswerStore();
                        if (was2 && typeof was2.processPracticeRecord === 'function' && data) {
                            await was2.processPracticeRecord(data);
                        }
                    }
                } catch (waErr) {
                    console.warn('[DataCollection] 等待错题写入失败（不影响主流程）:', waErr);
                }
        ...
        async showRealCompletionNotification(examId, realData) {
            ...
            if (scoreInfo) {
                accuracy = scoreInfo.percentage || Math.round((scoreInfo.accuracy || 0) * 100);
                correct = scoreInfo.correct || 0;
                total = scoreInfo.total || 0;
                duration = Math.round(realData.duration / 60);
            } else {
                duration = Math.round(realData.duration / 60);
            }
            ...
            const existingOverlay = document.getElementById('completion-result-overlay');
            if (existingOverlay) existingOverlay.remove();

            const overlay = document.createElement('div');
            overlay.id = 'completion-result-overlay';
            overlay.style.cssText = 'position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;animation:completionFadeIn 0.3s ease;';

            let scoreLine = '';
            if (total > 0) {
                scoreLine = '<div class="completion-stat-row"><span class="completion-stat-label">得分</span><span class="completion-stat-value">' + correct + '/' + total + '</span></div>';
                scoreLine += '<div class="completion-stat-row"><span class="completion-stat-label">正确率</span><span class="completion-stat-value">' + accuracy + '%</span></div>';
            }
            if (hasWrong) {
                scoreLine += '<div class="completion-stat-row completion-stat-row--wrong"><span class="completion-stat-label">错题数</span><span class="completion-stat-value completion-stat-value--wrong">' + wrongCount + '</span></div>';
            }
            scoreLine += '<div class="completion-stat-row"><span class="completion-stat-label">用时</span><span class="completion-stat-value">' + duration + ' 分钟</span></div>';

            let actionButtons = '<button class="completion-btn completion-btn--primary" id="completion-close-btn">我知道了</button>';
            if (hasWrong) {
                actionButtons = '<div class="completion-action-group">' +
                    '<button class="completion-btn completion-btn--primary" id="completion-wrongbook-btn">前往错题本重练</button>' +
                    '<button class="completion-btn completion-btn--ai" id="completion-ai-btn">AI 智能分析</button>' +
                    '</div>' +
                    '<button class="completion-btn completion-btn--secondary" id="completion-close-btn">关闭</button>';
            }

            overlay.innerHTML = '<div class="completion-result-card">' +
                '<div class="completion-result-icon">' + (accuracy >= 70 ? '🎉' : (accuracy >= 50 ? '💪' : '📖')) + '</div>' +
                '<h3 class="completion-result-title">练习完成</h3>' +
                '<p class="completion-result-exam">' + (exam.title || examId) + '</p>' +
                '<div class="completion-stats">' + scoreLine + '</div>' +
                (hasWrong ? '<p class="completion-hint">发现 ' + wrongCount + ' 道错题，可前往错题本重练或使用 AI 分析错因</p>' : '<p class="completion-hint completion-hint--perfect">全部正确，继续保持！</p>') +
                '<div class="completion-actions">' + actionButtons + '</div>' +
                '</div>';
            ...
            var closeBtn = overlay.querySelector('#completion-close-btn');
            if (closeBtn) closeBtn.addEventListener('click', function() { overlay.remove(); });

            if (hasWrong) {
                var wrongbookBtn = overlay.querySelector('#completion-wrongbook-btn');
                if (wrongbookBtn) wrongbookBtn.addEventListener('click', function() {
                    overlay.remove();
                    if (typeof window.showView === 'function') window.showView('wrongbook');
                    if (window.WrongBookView && typeof window.WrongBookView.scheduleRefresh === 'function') {
                        window.WrongBookView.scheduleRefresh(0);
                    }
                });

                var aiBtn = overlay.querySelector('#completion-ai-btn');
                if (aiBtn) aiBtn.addEventListener('click', function() {
                    overlay.remove();
                    if (typeof window.showView === 'function') window.showView('wrongbook');

                    var aiAttempts = 0;
                    var maxAiAttempts = 20;
                    var aiInterval = setInterval(function() {
                        aiAttempts++;
                        var aiActionBtn = document.querySelector('[data-wrongbook-action="ai-analyze"][data-exam-id="' + examId + '"]');
                        if (aiActionBtn) {
                            clearInterval(aiInterval);
                            aiActionBtn.click();
                        } else if (aiAttempts >= maxAiAttempts) {
                            clearInterval(aiInterval);
                            var allAnalysisBtn = document.getElementById('wrongbook-all-analysis-btn');
                            if (allAnalysisBtn) allAnalysisBtn.click();
                        }
                    }, 500);
            ...
            if (rec) {
                ...
                savePracticeRecordFallback(rec.examId, payload).finally(() => {
                    ...
                    if (typeof window.dispatchEvent === 'function' && typeof CustomEvent !== 'undefined') {
                        window.dispatchEvent(new CustomEvent('wrongbook:data-changed', { detail: { source: 'practice-complete-fallback' } }));
                    }
                });
            } else {
    ...
    try {
            ...
            try {
                if (typeof window.getWrongAnswerStore === 'function') {
                    ...
                    if (was && typeof was.processPracticeRecord === 'function') {
                        was.processPracticeRecord(canonicalRecord).catch(function (e) {
                            console.warn('[Fallback] 错题收集处理失败(PracticeCore):', e);
                        });
                    }
                }
            } catch (waErr) {
                console.warn('[Fallback] 错题收集异常(PracticeCore):', waErr);
            }
        ...
        try {
            if (typeof window.getWrongAnswerStore === 'function') {
                var was = window.getWrongAnswerStore();
                if (was && typeof was.processPracticeRecord === 'function') {
                    was.processPracticeRecord(canonicalRecord || record).catch(function (e) {
                        console.warn('[Fallback] 错题收集处理失败:', e);
                    });
                }
            }
        } catch (waErr) {
            console.warn('[Fallback] 错题收集异常:', waErr);
        }
    } catch (e) {
